...
export default function Sidebar() {
  ...
  const filteredCategories = Object.entries(categories).filter(([categorySlug, category]) => {
      ...
      <div className="p-4 h-screen overflow-y-auto">
        ...
        <ul className="space-y-4 mt-4">
          {filteredCategories.map(([categorySlug, category]) => (
            <li key={categorySlug}>
              <Link href={`/${categorySlug}`} className="text-xl font-semibold text-white/95 pt-4">
                <p>{category.name}</p>
              </Link>
              <ul className="pt-3">
